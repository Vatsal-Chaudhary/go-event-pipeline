...
type Archiver interface {
	Enqueue(status string, eventData interface{}) error
	Close() error
}

type objectUploader interface {
	Upload(ctx context.Context, bucket, objectPath string, payload []byte) error
}
...
func NewBatchArchiver(uploader objectUploader, bucket string, batchSize int, flushInterval time.Duration, prefixMode string) (*BatchArchiver, error) {
	if uploader == nil {
...
func (b *BatchArchiver) uploadBatch(ctx context.Context, status string, records [][]byte) error {
	...
	if err := b.uploader.Upload(ctx, b.bucket, objectPath, compressed); err != nil {
		return err
	}
